...
PACK_DIRECTORY=/etc/puppet/modules/wso2am_analytics/files
WSO2_SERVER_PACK=wso2am-analytics-2.2.0.zip
WSO2_SERVER_UPDATED_PACK=wso2am-analytics-2.2.0.*.zip
WUM_HOME=/usr/local/
WUM_ARCHIVE=wum-2.0-linux-x64.tar.gz
WUM_PATH='PATH=$PATH:/usr/local/wum/bin'
WUM_PRODUCT_LOCATION=/home/ubuntu/.wum-wso2/products/wso2am-analytics/2.2.0
...
if [ ! -f ${WUM_PRODUCT_LOCATION}/${WSO2_SERVER_UPDATED_PACK} ];then
  ...
  sudo mv ${WUM_PRODUCT_LOCATION}/${WSO2_SERVER_UPDATED_PACK} /etc/puppet/modules/wso2am_analytics/files/${WSO2_SERVER_PACK}
fi
