...
apt-get install -y postgresql-client alien mssql-tools unixodbc-dev
apt-get install -y mariadb-client
ln -sfn /opt/mssql-tools/bin/sqlcmd /usr/bin/sqlcmd
ln -sfn /opt/mssql-tools/bin/bcp /usr/bin/bcp

# Install oracle clients using alien packages
...
wget https://product-dist.wso2.com/downloads/wum/1.0.0/wum-1.0-linux-x64.tar.gz -P /home/ubuntu/
...
if [ "$CF_PRODUCT" == "APIM" ]; then
  ...
  # Copy crontab to run init.sh @reboot
  cp /tmp/crontab /etc/crontab
  cp /tmp/init.sh /home/ubuntu/init.sh
...
mv /opt/java/jre/lib/security/local_policy.jar /opt/java/jre/lib/security/local_policy.jar.orig
mv /opt/java/jre/lib/security/US_export_policy.jar /opt/java/jre/lib/security/US_export_policy.jar.orig
